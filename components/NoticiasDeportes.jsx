...
export function NoticiasDeportes () {

  const [noticias, setNoticias] = useState([]);
  const noticiasDeDeportes = noticias.filter(noticia => noticia.categoria === 'Deporte');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeDeportes.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeDeportes.length / noticiasPorSlide);
