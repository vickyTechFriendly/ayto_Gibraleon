...
export function NoticiasSeguridad () {
  
  const [noticias, setNoticias] = useState([]);
  const noticiasDeSeguridad = noticias.filter(noticia => noticia.categoria === 'Seguridad');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeSeguridad.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeSeguridad.length / noticiasPorSlide);
