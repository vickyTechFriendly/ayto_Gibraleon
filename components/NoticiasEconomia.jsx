...
export function NoticiasEconomia () {
  ...
  const noticiasDeEconomia = noticias.filter(noticia => noticia.categoria === 'Economía');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeEconomia.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeEconomia.length / noticiasPorSlide);
