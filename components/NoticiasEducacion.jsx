...
export function NoticiasEducacion () {
  
  const [noticias, setNoticias] = useState([]);
  const noticiasDeEducacion = noticias.filter(noticia => noticia.categoria === 'Educación');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeEducacion.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeEducacion.length / noticiasPorSlide);
