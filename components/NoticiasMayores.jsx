...
export function NoticiasMayores () {
  
  const [noticias, setNoticias] = useState([]);
  const noticiasDeMayores = noticias.filter(noticia => noticia.categoria === 'Mayores');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeMayores.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeMayores.length / noticiasPorSlide);
