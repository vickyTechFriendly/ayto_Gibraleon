...
export function NoticiasUrbanismo () {
  
  const [noticias, setNoticias] = useState([]);
  const noticiasDeUrbanismo = noticias.filter(noticia => noticia.categoria === 'Urbanismo');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeUrbanismo.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeUrbanismo.length / noticiasPorSlide);
