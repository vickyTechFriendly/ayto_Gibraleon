...
export function NoticiasServiciosSociales () {
  
  const [noticias, setNoticias] = useState([]);
  const noticiasDeServiciosSociales = noticias.filter(noticia => noticia.categoria === 'Servicios Sociales');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeServiciosSociales.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeServiciosSociales.length / noticiasPorSlide);
