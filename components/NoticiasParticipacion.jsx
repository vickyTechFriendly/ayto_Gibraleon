...
export function NoticiasParticipacion () {
  
  const [noticias, setNoticias] = useState([]);
  const noticiasDeParticipacion = noticias.filter(noticia => noticia.categoria === 'Participación ciudadana');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeParticipacion.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeParticipacion.length / noticiasPorSlide);
