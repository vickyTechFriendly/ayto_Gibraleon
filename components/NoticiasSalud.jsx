...
export function NoticiasSalud () {
  
  const [noticias, setNoticias] = useState([]);
  const noticiasDeSalud = noticias.filter(noticia => noticia.categoria === 'Salud');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeSalud.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeSalud.length / noticiasPorSlide);
