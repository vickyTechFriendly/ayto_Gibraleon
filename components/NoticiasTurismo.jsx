...
export function NoticiasTurismo () {
  
  const [noticias, setNoticias] = useState([]);
  const noticiasDeTurismo = noticias.filter(noticia => noticia.categoria === 'Turismo');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeTurismo.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeTurismo.length / noticiasPorSlide);
