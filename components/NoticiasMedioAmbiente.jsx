...
export function NoticiasMedioAmbiente () {
  
  const [noticias, setNoticias] = useState([]);
  const noticiasDeMedioAmbiente = noticias.filter(noticia => noticia.categoria === 'Medio Ambiente');
  const noticiasPorSlide = 3;
  const isLessThanThreeNews = noticiasDeMedioAmbiente.length <= 3;
  const noticiasMovilStyle = isLessThanThreeNews ? { padding: '0px' } : {};
  const totalSlides = Math.ceil(noticiasDeMedioAmbiente.length / noticiasPorSlide);
